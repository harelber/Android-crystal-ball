...
def change_manifest_application(app):
	try:
		...
		for i in range(0,len(con)):
			#if "<application " in con[i] and "android:name" not in con[i]:
				#con[i]=con[i].replace('<application ','<application android:name="PACK.MyApplication" ')
				#con[i]=con[i].replace('PACK',pack[1:-1])
			#	break
                        pack=''
			if "<manifest" in con[i] and "package" in con[i]:
				pack=con[i].split('package="')[1].split('"')[0]
                        else:
                                if "<activity" in con[i]:
                                        pack=".".join(con[i].split('name="')[1].split('"')[0].split(".")[:-1])
                        if pack:
                                packages[pack]=1
		...
		#w.close()
		        path=app+"/smali/"+pack.replace(".","/")
		                ...
		                with open(path+"/MyApplication.smali") as f:
		                    newText=f.read().replace('X/', "L"+pack.replace(".","/")+"/")
...
def change_manifest_intent_actions(intent_actions,package_name):
	print package_name
	intents_check=[]
        if not os.path.exists(package_name+"/assets/"):
